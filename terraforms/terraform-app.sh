...
run_terraform() {
  ...
  if [ $? -ne 0 ]; then
	...
	echo "{\"status\":\"failed\"}" > $CONFIG/failure_$1.json
  else
	log $TIMESTAMP “Success”
	echo "{\"status\":\"success\"}" > $CONFIG/success_$1.json
  fi
}
...
run_destroy() {
  ...
  run_terraform destroy -input=false -auto-approve=true -var-file=$CONFIG/rds.tfvars.json -var-file=$CONFIG/ec2.tfvars.json $CONFIG 2>$CONFIG/error_destroy.log
}
